...
ProwJobs can also be manually triggered by generating a YAML ProwJob CRD resource
with mkpj and deploying that directly to the Prow cluster, however this pattern
is generally not reccomended. It requires the use of direct prod cluster access,
...
properly.
These jobs are best run locally as decorated pods inside a [Kind] cluster [Using pj-on-kind.sh](#using-pj-on-kind.sh).
